...
namespace InvoiceApi.Services
{
    public class TestService : ITestService
    {
        ...
        public TestService(INopDbContext2 nopDbContext)
        {
            _nopDbContext = nopDbContext;
        }
        public byte[] inHoadonTest()
        {
            var objDataSet = new DataSet();
            var path = "~/Content/Test.xml";
            var file = System.Web.HttpContext.Current.Server.MapPath(path);
            objDataSet.ReadXml(file);
            DataSet ds = new DataSet();
            ds = objDataSet;
            var report_file = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/Report/") + "\\" + "123.repx";
            var report = Util.ReportUtil.PrintReport(ds, report_file, "PDF");
            return report;
        ...
        #region 204 có chi tiết lấy bang 1
        public byte[] inHoadon1()
        {
            //var objDataSet = new DataSet();
            //var path = "~/Content/Test.xml";
            //var file = System.Web.HttpContext.Current.Server.MapPath(path);
            ...
            //DataSet ds = new DataSet();
            //ds = objDataSet;
            //var report_file = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/Report/") + "\\" + "01TB_KTDL_TH2_KHL.repx";
            //var report = Util.ReportUtil.PrintReport1(ds, report_file, "PDF");
            //return report;


            var objDataSet = new DataSet();
            //var path = @"D:\M-Invoice\ProjectTraCuuMinvoice\1308\TracuuhoadonAPI-master\InvoiceApi\Content\Test.xml";
            var path = @"D:\M-Invoice\ProjectTraCuuMinvoice\26032022\API\TracuuhoadonAPI-master\InvoiceApi\Content\Test.xml";
            var xml = objDataSet.ReadXml(path.ToString());
            ...
            XmlNodeList MGDDTu = xmlDocument.GetElementsByTagName("MGDDTu");
            ...
            if (MGDDTu.Count > 0)
            {
                TTHDon.Rows[0]["mGDDTu"] = MGDDTu[0].InnerText;
            }
            DataSet ds = new DataSet();
            ...
            var col2 = new List<DataColumn>() {
                                            new DataColumn()
                                            {
                                                ColumnName= "x509SubjectName"
                                            },
                                            new DataColumn()
                                            {
                                                ColumnName= "signingTime"
                                            }
            ...
            //XmlNodeList list = xmlDocument.SelectNodes("/TDiep/DLieu/TBao/DLTBao/LCMa/DSLDo/LDo");
            ...
            var report_file = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/Report/") + "\\" + "ThongDiep_204.repx";
            var report = Util.ReportUtil.PrintReport1(ds, report_file, "PDF");
        ...
        public byte[] inHoadon2()
        {
            ...
            var report_file = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/Report/") + "\\" + "ThongDiep_103.repx";
            var report = Util.ReportUtil.PrintReport(ds, report_file, "PDF");
            return report;
        }
        #endregion
        ...
        #region 102 có chi tiết lấy bang 1
        public byte[] inHoadon3()
        {
            ...
            #region tiếp nhận và không tiếp nhận đăng ký
            XmlNodeList So = xmlDocument.GetElementsByTagName("So");
            XmlNodeList NTBao = xmlDocument.GetElementsByTagName("NTBao");
            XmlNodeList MST = xmlDocument.GetElementsByTagName("MST");
            XmlNodeList TNNT = xmlDocument.GetElementsByTagName("TNNT");
            ...
            var report_file = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/Report/") + "\\" + "ThongDiep_102.repx";
            var report = Util.ReportUtil.PrintReport1(ds, report_file, "PDF");
            return report;
        }
        ...
        #region 301 có chi tiết lấy bang 1
        public byte[] inHoadon4()
        {
            ...
            XmlNodeList nodeList = xmlDocument.GetElementsByTagName("X509SubjectName");
            //XmlNodeList list = xmlDocument.SelectNodes("/TDiep/DLieu/TBao/DLTBao/LCMa/DSLDo/LDo");
            ...
            var report_file = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/Report/") + "\\" + "ThongDiep_301.repx";
            var report = Util.ReportUtil.PrintReport(ds, report_file, "PDF");
            return report;
        }
        ...
        #region 202 có chi tiết
        public byte[] inHoadon5()
        {
            ...
            XmlNodeList Ten = xmlDocument.GetElementsByTagName("Ten");
            XmlNodeList MST = xmlDocument.GetElementsByTagName("MST");
            XmlNodeList DChi = xmlDocument.GetElementsByTagName("DChi");
            XmlNodeList SDThoai = xmlDocument.GetElementsByTagName("SDThoai");
            XmlNodeList DCTDTu = xmlDocument.GetElementsByTagName("DCTDTu");
            ...
            if (DChi.Count > 0)
            {
                TTHDon.Rows[0]["dChi"] = DChi[0].InnerText;
            }
            if (SDThoai.Count > 0)
            {
                TTHDon.Rows[0]["sDThoai"] = SDThoai[0].InnerText;
            }
            if (DCTDTu.Count > 0)
            {
                TTHDon.Rows[0]["dCTDTu"] = DCTDTu[0].InnerText;
            }
            if (TSuat.Count > 0)
            {
            ...
            XmlNodeList TLCKhau = xmlDocument.GetElementsByTagName("TLCKhau");
            XmlNodeList STCKhau = xmlDocument.GetElementsByTagName("STCKhau");
            ...
            for (int i = 0; i < nodeList.Count; i++)
            {
                ...
                if (TLCKhau.Count > 0)
                {
                    dSHH.Rows[i]["tLCKhau"] = TLCKhau[i].InnerText;
                }
            ...
            var report_file = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/Report/") + "\\" + "ThongDiep_202.repx";
            var report = Util.ReportUtil.PrintReport1(ds, report_file, "PDF");
            return report;
        }
        #endregion
        ...
        #region
        public byte[] PrintThongDiep1()
        {
            byte[] array = null;
            try
            {
                var objDataSet = new DataSet();
                var path = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/") + "\\" + "Test.xml";

                XmlDocument xmlDocument = new XmlDocument();
                xmlDocument.PreserveWhitespace = true;
                xmlDocument.Load(path);
                string xmlcontents = xmlDocument.InnerXml;
                string type = xmlDocument.GetElementsByTagName("MLTDiep")[0].InnerText.ToString();
                ...
                if (type == "204")
                {
                    ...
                    #region danh sách lỗi hóa đơn, ss04,
                ...
                if (type == "204")
                {
                    ten_mau = "ThongDiep_204.repx";

                }
                else if (type == "202")
                {
                    ten_mau = "ThongDiep_202.repx";

                }
                else if (type == "102")
                {
                    ten_mau = "ThongDiep_102.repx";

                }
                else if (type == "103")
                {
                    ten_mau = "ThongDiep_103.repx";

                }
                else if (type == "301")
                {
                    ten_mau = "ThongDiep_301.repx";

                }
                var folder = System.Web.Hosting.HostingEnvironment.MapPath("~/Content/Report/") + "\\" + ten_mau.ToString();
                array = Util.ReportUtil.PrintReport(ds, folder, "PDF", 1);
                if (array == null)
                {
        ...
        private DataSet InTD202(XmlDocument xmlDocument, DataSet ds)
        {
            ...
            string TNHangNM = "";
            string HVTNMHang = "";
            if (xmlDocument.GetElementsByTagName("Ten")[1] != null)
                TenNM = xmlDocument.GetElementsByTagName("Ten")[1].InnerText.ToString();
            if (xmlDocument.GetElementsByTagName("MST")[2] != null)
                MSTNM = xmlDocument.GetElementsByTagName("MST")[2].InnerText.ToString();
            if (xmlDocument.GetElementsByTagName("DChi")[1] != null)
                DChiNM = xmlDocument.GetElementsByTagName("DChi")[1].InnerText.ToString();
            if (xmlDocument.GetElementsByTagName("SDThoai")[1] != null)
                SDThoaiNM = xmlDocument.GetElementsByTagName("SDThoai")[1].InnerText.ToString();
            if (xmlDocument.GetElementsByTagName("DCTDTu")[1] != null)
                DCTDTuNM = xmlDocument.GetElementsByTagName("DCTDTu")[1].InnerText.ToString();
            ...
            dSHH.Columns.AddRange(col1.ToArray());
            XmlNodeList nodeList = xmlDocument.GetElementsByTagName("HHDVu");
            SLuong[0].ParentNode.RemoveChild(SLuong[0]);
            for (int i = 0; i < nodeList.Count; i++)
            {
                DataRow dr1 = dSHH.NewRow();
                dSHH.Rows.Add(dr1);
